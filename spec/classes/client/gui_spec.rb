    ...
    context "on #{os}" do
      ...
      context 'when using defaults' do
        ...
        if os_facts[:os]['family'] == 'RedHat'
          it { is_expected.to contain_package('tigervnc').with_ensure('installed') }
        elsif os_facts[:os]['family'] == 'Debian'
          it { is_expected.to contain_package('tigervnc-viewer').with_ensure('installed') }
        else
